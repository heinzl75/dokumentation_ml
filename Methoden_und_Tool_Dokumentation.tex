...
\fancyhf{}
\fancyhead[L]{\small\nouppercase{\leftmark}}
\fancyhead[R]{\small\textit{LMNTool -- Methodendokumentation}}
\fancyfoot[C]{\thepage}
...
pdfauthor={heinzl75},
pdfsubject={HILOMOT, LOLIMOT, MLP, SVR, GPR, LMNTool}
}
...
anhand des LMNTool Multi GUI}\\[8pt]
...
\vfill
\begin{center}
{\small Erstellt mit \LaTeX{} und Biber/biblatex. Abbildungen aus dem LMNTool Multi GUI.}
\end{center}
...
\cite{rasmussen2006}. Alle f\"{u}nf Methoden sind im \emph{LMNTool Multi GUI} implementiert
...
\item \textbf{Implementierung:} Die konkrete Umsetzung in zwei Werkzeugen wird beschrieben:
      im Python-Tool \emph{LMNTool Multi GUI} (f\"{u}nf-Methoden-Vergleich) sowie in
...
\item[Kapitel~9] LMNTool Multi GUI -- Python-Vergleichswerkzeug, Datenimport, Parameter, Visualisierungen, Export.
\item[Kapitel~10] MeinWindowLMNTool -- MATLAB-App-Designer-Werkzeug f\"{u}r HILOMOT/LOLIMOT: Aufbau, Bedienung, Visualisierungen, Export und Einordnung.
\item[Kapitel~11] Vergleich aller f\"{u}nf Methoden.
\item[Kapitel~12] Diskussion.
\item[Kapitel~13] Zusammenfassung und Ausblick.
\end{description}
...
\caption{3D-Ansicht der Trainingsdaten nach dem CSV-Import im LMNTool. Die Zielgr\"{o}\ss{}e $y$
...
im LMNTool als CSV-Datei importiert (Spaltenreihenfolge: $y$, $x_1$, $x_2$) und f\"{u}r alle
...
Zur Bewertung der Modellg\"{u}te werden im LMNTool die folgenden standardisierten Ma\ss{}e
...
Typisch sind $k=5$ oder $k=10$. Das LMNTool unterst\"{u}tzt $k$-fache Kreuzvalidierung zur
...
in der Praxis empfehlenswert. Das LMNTool normiert die Daten intern vor der \"{U}bergabe an die
sklearn-Sch\"{a}tzer.
...
Das Python-Tool \emph{LMNTool Multi GUI} orientiert sich an dieser Schnittstelle.

\section{Hyperparameter von LOLIMOT}

\begin{table}[H]
\centering
\caption{Wichtige Hyperparameter von LOLIMOT im LMNTool}
...
\section{Antwortfl\"{a}che (LOLIMOT)}

\begin{figure}[H]
\centering
\includegraphics[width=0.68\textwidth]{surface_lolimot.png}
\caption{Modellierte Antwortfl\"{a}che (LOLIMOT). Im Vergleich zu HILOMOT k\"{o}nnen achsenparallele
Splits bei schr\"{a}g orientierten Strukturen leicht h\"{o}here Stufenm\"{a}\ss{}igkeit erzeugen.
Die G\"{u}ltigkeitsfunktionen gew\"{a}hrleisten aber immer eine global glatte Fl\"{a}che.}
...
\begin{table}[H]
\centering
\caption{Wichtige Hyperparameter von HILOMOT im LMNTool}
...
Im LMNTool wird \texttt{MLPRegressor} aus scikit-learn verwendet \cite{sklearn_mlp, pedregosa2011}.
...
\begin{table}[H]
\centering
\caption{Wichtige Hyperparameter des MLP im LMNTool (\texttt{MLPRegressor})}
...
\cite{matlab_svr}. Im LMNTool wird scikit-learn's \texttt{SVR} eingesetzt
...
\begin{table}[H]
\centering
\caption{Wichtige Hyperparameter der SVR im LMNTool}
...
MATLAB bietet GPR \"{u}ber \texttt{fitrgp} \cite{matlab_gpr}. Im LMNTool wird
...
-\frac{N}{2}\log(2\pi)
\end{equation}
mit $K_y = K + \sigma_n^2 I$. Das LMNTool f\"{u}hrt \texttt{n\_restarts\_optimizer} Neustarts
...
\begin{table}[H]
\centering
\caption{Wichtige Hyperparameter der GPR im LMNTool}
...
% ============================================================
\chapter{LMNTool Multi GUI}
% ============================================================

\section{\"{U}berblick und Architektur}

LMNTool Multi GUI ist ein Python-basiertes Vergleichswerkzeug f\"{u}r f\"{u}nf Regressionsverfahren
...
\section{Datenimport und Vorverarbeitung}
...
\caption{3D-Datenvorschau im LMNTool nach dem CSV-Import. Die Visualisierung erm\"{o}glicht
...
\section{Methoden-Tabs und Parametrierung}
...
\begin{table}[H]
\centering
\caption{Zusammenfassung der Kernparameter aller Methoden im LMNTool}
...
\section{Training und Bewertungslogik}
...
\caption{Korrelationsplot im LMNTool: gemessene Zielwerte $y$ vs.\ Modellvorhersage $\hat{y}$.
...
\section{Report- und Datenexport}
...
% ============================================================
\chapter{MeinWindowLMNTool}
% ============================================================

\section{\"{U}berblick und Einordnung}
...
\begin{merkbox}
\textbf{Einordnung:} W\"{a}hrend das \emph{LMNTool Multi GUI} (Kapitel~9) ein
...
\begin{table}[H]
\centering
\caption{Gegen\"{u}berstellung: MeinWindowLMNTool vs.\ LMNTool Multi GUI}
\label{tab:tools_vergleich}
\begin{tabular}{p{4.0cm}p{5.2cm}p{5.2cm}}
\toprule
\textbf{Merkmal} & \textbf{MeinWindowLMNTool} & \textbf{LMNTool Multi GUI} \\
\midrule
...
\section{Programmaufbau und Startvoraussetzungen}
...
\section{Methoden- und Parameterwahl}
...
\section{Export und Integration}
...
\section{Vergleich mit dem Python LMNTool Multi GUI}
...
\item[LMNTool Multi GUI (Python):] Ideal f\"{u}r plattform\"{u}bergreifenden Einsatz und
...
\toprule
\textbf{Funktion} & \textbf{MeinWindowLMNTool} & \textbf{LMNTool Multi GUI} \\
\midrule
...
bietet das \emph{LMNTool Multi GUI} den vollst\"{a}ndigeren Rahmen.
\end{merkbox}
...
\item[HILOMOT und LOLIMOT] bilden die Struktur durch explizite Partitionierung ab. Die
    Partitionierungskarte (Abb.~\ref{fig:partition_map}) zeigt klar, welche Teilmodelle
...
Implementierung im LMNTool Multi GUI ausf\"{u}hrlich beschrieben. Die zentralen Erkenntnisse:
...
\item \textbf{Das LMNTool Multi GUI} erm\"{o}glicht einen direkten, reproduzierbaren Vergleich
