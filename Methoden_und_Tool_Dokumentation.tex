...
Alle Methoden werden theoretisch hergeleitet und anhand eines realen Datensatzes
...
\item \textbf{Theorie:} Die mathematischen Grundlagen aller f\"{u}nf Verfahren werden
      ...
      und Online-Dokumentationen (scikit-learn~\cite{sklearn_mlp,sklearn_svr,sklearn_gpr}).

\item \textbf{Implementierung:} Die konkrete Umsetzung im Python-Tool \emph{L2ML-Fitting-Tool}
      (f\"{u}nf-Methoden-Vergleich) wird beschrieben, einschlie\ss{}lich aller relevanten
      Hyperparameter und ihrer Wirkung.
...
\item[Kapitel~9] L2ML-Tool -- Python-Vergleichswerkzeug: Datenimport, Parameter, Visualisierungen, Export.
...
% ------------------------------------------------------------
\section{Allgemeiner Fit-Workflow im L2ML-Fitting-Tool}
\label{sec:general_fit_workflow}

Unabh\"{a}ngig vom gew\"{a}hlten Regressionsverfahren folgt das Training im L2ML-Fitting-Tool einem
einheitlichen Ablauf. Dadurch sind die Ergebnisse der unterschiedlichen Sch\"{a}tzer direkt
vergleichbar und Unterschiede lassen sich auf Modellannahmen und Hyperparameter zur\"{u}ckf\"{u}hren.

\subsection{Datenfluss: von CSV zu trainiertem Modell}

Der grundlegende Fit-Prozess besteht aus den folgenden Schritten:

\begin{enumerate}[label=\textbf{Schritt \arabic*:}, leftmargin=2.2cm]
\item \textbf{Datenimport und Plausibilisierung:}
      Einlesen der CSV-Datei, Pr\"{u}fung der Spaltenreihenfolge sowie Visualisierung
      (z.\,B.\ 3D-Plot). Optional werden ung\"{u}ltige Zeilen (NaN, Inf) entfernt.

\item \textbf{Train/Test-Split und Reproduzierbarkeit:}
      Aufteilung in Trainings- und Testdaten (Holdout) bzw.\ Wahl einer $k$-fachen
      Kreuzvalidierung (\texttt{KFold}, $k = 5$ oder $k = 10$).
      F\"{u}r Vergleichbarkeit wird ein fester Zufalls-Seed verwendet.

\item \textbf{Skalierung der Eingansgr\"{o}\ss{}en (und ggf.\ des Outputs):}
      Standardisierung der Eingangsvariablen $x_j$ gem\"{a}\ss{}
      $\tilde{x}_j = (x_j-\mu_j)/\sigma_j$.
      Dies ist insbesondere f\"{u}r SVR, GPR und MLP entscheidend, da Kernelbreiten,
      L\"{a}ngenma\ss{}st\"{a}be und Lernraten skalenabh\"{a}ngig sind.

\item \textbf{Hyperparameterwahl mittels Gridsearch und Kreuzvalidierung:}
      Entweder manuell (Tool-UI) oder automatisiert per Grid-Search mit Kreuzvalidierung.
      Das L2ML-Fitting-Tool unterst\"{u}tzt \texttt{GridSearchCV} aus scikit-learn, wobei
      der Parameterraum methodenspezifisch gew\"{a}hlt wird (z.\,B.\ $C$, $\gamma$, $\varepsilon$
      bei SVR; \texttt{max\_leafs}, \texttt{smoothness} bei LOLIMOT/HILOMOT).
      Die Wahl der Hyperparameter beeinflusst Bias/Varianz und numerische Stabilit\"{a}t.

\item \textbf{Modell-Fit (Training):}
      Sch\"{a}tzung der Modellparameter durch Minimierung eines Trainingskriteriums
      (z.\,B.\ MSE, regularisierte Loss, marginale Likelihood).

\item \textbf{Evaluation und Diagnose:}
      Berechnung von MSE/MAE/RMSE/$R^2$ auf Train und Test.
      Zus\"{a}tzlich werden Diagnoseplots erzeugt (Residuals, Oberfl\"{a}che,
      Partitionierungskarte).

\item \textbf{Export/Deployment-Artefakte:}
      Speichern von Modellparametern (z.\,B.\ LMN-Parameter, Support Vectors,
      Netzgewichte), sowie ggf.\ Skalierungsparametern $\mu,\sigma$, um die Vorhersage
      im Zielsystem korrekt reproduzieren zu k\"{o}nnen.
\end{enumerate}

\begin{hinweisbox}
In den folgenden Methoden-Kapiteln wird dieser generelle Workflow jeweils konkretisiert:
Welche Parameter werden gesch\"{a}tzt? Welches Optimierungsproblem wird gel\"{o}st?
Welche Abbruchkriterien und numerischen Besonderheiten existieren?
\end{hinweisbox}
% ------------------------------------------------------------
...
% ------------------------------------------------------------
\subsection{Fit-Prozess im Detail: LOLIMOT}
\label{sec:lolimot_fit_process}

Der Fit-Prozess von LOLIMOT kann als wiederholte Folge aus \emph{(i) Fehlerlokalisierung},
\emph{(ii) Kandidaten-Split}, \emph{(iii) Re-Fit der lokalen Modelle} beschrieben werden.

\subsubsection{1) Initialer Fit (Startmodell)}
Zu Beginn existiert genau ein Blatt ($M=1$). Das lokale lineare Modell
$\hat{y}_1(\mathbf{x})=\boldsymbol{\theta}_1^T\tilde{\mathbf{x}}$ wird per OLS gesch\"{a}tzt.
Die G\"{u}ltigkeit ist trivial: $\Phi_1(\mathbf{z}) \equiv 1$.

\subsubsection{2) Fehlerzuweisung auf Bl\"{a}tter}
F\"{u}r jedes Blatt $i$ wird ein lokaler Fehler $e_i$
(Gleichung~\ref{eq:lolimot_local_error}) berechnet. Datenpunkte tragen umso st\"{a}rker
zu $e_i$ bei, je gr\"{o}\ss{}er $\Phi_i(\mathbf{z}_k)$ ist -- eine \emph{gewichtete}
Fehlerzuweisung.

\subsubsection{3) Generierung von Split-Kandidaten}
Das schlechteste Blatt $i^*$ wird entlang jeder Eingangsdimension $d$ geteilt.
Ein Kandidat ist durch $(d,c)$ definiert, wobei $c$ als Mittelpunkt des aktuell vom
Blatt abgedeckten Intervalls in Dimension $d$ gew\"{a}hlt wird.
F\"{u}r jeden Kandidaten werden die neuen G\"{u}ltigkeitsfunktionen \"{u}ber Sigmoids aufgebaut
(Kapitel~3), sodass der \"{U}bergang weich bleibt.

\subsubsection{4) Re-Fit der Blattmodelle mittels WLS}
Nach einem Kandidaten-Split entstehen zwei neue Bl\"{a}tter. Deren lokale lineare Parameter
werden per gewichteter kleinster Quadrate (WLS) bestimmt
(Gleichung~\ref{eq:wls}). Dabei ist entscheidend:
\begin{itemize}
\item Die Gewichtsmatrix $W_i$ h\"{a}ngt direkt von der G\"{u}ltigkeit $\Phi_i(\mathbf{z})$ ab.
\item Bei sehr kleinen G\"{u}ltigkeiten kann das WLS-Problem schlecht konditioniert werden;
      daher ist ein \texttt{demanded\_membership}-Schwellwert sinnvoll.
\end{itemize}

\subsubsection{5) Auswahl des besten Kandidaten (globaler MSE)}
F\"{u}r jeden Kandidaten wird das \emph{globale} Trainingskriterium (typisch MSE \"{u}ber alle
Daten) berechnet. Der Kandidat mit minimalem globalen Fehler wird gew\"{a}hlt. Danach wird
der Baum dauerhaft erweitert und der Prozess wiederholt.

\subsubsection{6) Abbruchkriterien und Hyperparameter-Effekte}
Typische Abbruchkriterien sind \texttt{max\_leafs} sowie eine minimale Fehlerverbesserung.
In der Praxis zeigt sich:
\begin{itemize}
\item H\"{o}here \texttt{smoothness} f\"{u}hrt zu stabileren, glatteren Modellen, kann aber die
      Trennsch\"{a}rfe reduzieren.
\item \texttt{min\_samples\_per\_leaf} wirkt als strukturelle Regularisierung und verhindert
      \"{u}berm\"{a}\ss{}ig kleine Regionen.
\item Im L2ML-Fitting-Tool kann die Hyperparameterwahl (z.\,B.\ \texttt{max\_leafs} und
      \texttt{smoothness}) per Grid-Search mit Kreuzvalidierung automatisiert erfolgen.
\end{itemize}
% ------------------------------------------------------------
...
% ------------------------------------------------------------
\subsection{Fit-Prozess im Detail: HILOMOT}
\label{sec:hilomot_fit_process}

HILOMOT folgt ebenfalls einem Tree-Growing-Ansatz, erweitert LOLIMOT jedoch um die
Optimierung der Split-Geometrie. W\"{a}hrend LOLIMOT pro Wachstumsschritt nur achsenparallele
Kandidaten pr\"{u}ft, enth\"{a}lt HILOMOT zus\"{a}tzlich eine kontinuierliche Optimierung
der Orientierung.

\subsubsection{1) Fehlerlokalisierung}
Analog zu LOLIMOT wird zun\"{a}chst ein ``schlechtestes'' Blatt $i^*$ bestimmt, typischerweise
anhand eines gewichteten Fehlers. Dadurch konzentriert sich das Wachstum auf Regionen, in
denen das Modell die Zielfunktion noch nicht gut erkl\"{a}rt.

\subsubsection{2) Initialisierung obliquer Splitrichtungen via gewichteter PCA}
Ist \texttt{oblique=True}, wird zus\"{a}tzlich zu den Achsenrichtungen eine datengetriebene
Start-Richtung bestimmt. Eine robuste Heuristik ist die PCA-Richtung der gewichtet im
Blatt liegenden Daten:
\begin{itemize}
\item Datenpunkte werden mit $\Phi_{i^*}(\mathbf{z}_k)$ gewichtet.
\item Aus der gewichteten Kovarianz wird die erste Hauptkomponente als Richtungsvektor
      gewonnen.
\end{itemize}
Diese Richtung dient als Startpunkt f\"{u}r eine lokale, kontinuierliche Optimierung.

\subsubsection{3) Kontinuierliche Winkeloptimierung (2D-Fall)}
Im zweidimensionalen Fall kann der Normalenvektor als
$\mathbf{w}(\theta)=[\cos\theta,\sin\theta]^T$ parametrisiert werden
(Gleichung~\ref{eq:hilomot_angle_opt}). F\"{u}r jeden Winkel $\theta$ ergibt sich eine andere
Sigmoid-Trennung und damit andere G\"{u}ltigkeiten $\Phi_i(\mathbf{z})$. Nach jedem
Kandidatenwinkel erfolgt ein Re-Fit der Blattparameter via WLS. Der optimale Winkel
minimiert das globale Trainingskriterium (z.\,B.\ MSE).

\subsubsection{4) WLS Re-Fit und Stabilit\"{a}t durch demanded\_membership}
Wie bei LOLIMOT werden die lokalen linearen Modelle nach einem Split per WLS gesch\"{a}tzt.
Der Parameter \texttt{demanded\_membership\_value} wirkt hier besonders stabilisierend,
da oblique Splits Regionen erzeugen k\"{o}nnen, in denen nur wenige Punkte mit hoher
G\"{u}ltigkeit liegen.

\subsubsection{5) Abbruchkriterien}
Neben \texttt{max\_leafs} sind folgende Kriterien praxisrelevant:
\begin{itemize}
\item minimale Verbesserung des globalen Fehlers,
\item minimale effektive Stichprobengr\"{o}\ss{}e pro Blatt,
\item Begrenzung der Optimierungsiterationszahl in der Winkeloptimierung.
\end{itemize}
Auch bei HILOMOT unterst\"{u}tzt das L2ML-Fitting-Tool Grid-Search mit Kreuzvalidierung zur
systematischen Suche nach optimalen Hyperparametern wie \texttt{max\_leafs},
\texttt{smoothness} und \texttt{demanded\_membership\_value}.

\begin{merkbox}
Der zentrale Unterschied im Fit-Prozess ist nicht die Sch\"{a}tzung der lokalen Modelle
(WLS), sondern die zus\"{a}tzliche Optimierung der \emph{Split-Geometrie}. Diese erh\"{o}ht
den Fit-Aufwand pro Wachstumsschritt, kann aber die notwendige Blattanzahl deutlich
reduzieren.
\end{merkbox}
% ------------------------------------------------------------
...
% ------------------------------------------------------------
\subsection{Fit-Prozess im Detail: MLPRegressor (scikit-learn)}
\label{sec:mlp_fit_process}

Der Fit-Prozess des MLP unterscheidet sich grundlegend von LMN-Verfahren: Es existiert
keine explizite Partitionierung des Eingangsraums, sondern eine globale parametrisierte
Funktion $\hat{f}(\mathbf{x};\boldsymbol{\theta})$, deren Parameter iterativ optimiert
werden.

\subsubsection{1) Datenvorbereitung und Skalierung}
Vor dem Fit ist eine Skalierung der Eing\"{a}nge nahezu zwingend:
\begin{itemize}
\item Ohne Skalierung k\"{o}nnen Gradienten sehr unterschiedlich skaliert sein, was zu
      langsamer oder instabiler Konvergenz f\"{u}hrt.
\item Auch die Regularisierung \"{u}ber \texttt{alpha} wirkt dann schwer interpretierbar.
\end{itemize}

\subsubsection{2) Initialisierung der Gewichte}
Die Gewichte $W^{(l)}$ werden zuf\"{a}llig initialisiert (z.\,B.\ nach Xavier/Glorot).
Die Wahl der Initialisierung beeinflusst, ob Aktivierungen in S\"{a}ttigung geraten
(z.\,B.\ bei Sigmoid/Tanh) und damit das Auftreten von Vanishing Gradients.

\subsubsection{3) Iterative Optimierung (Epochen)}
Pro Epoche wird die Verlustfunktion (MSE + Regularisierung,
Gleichung~\ref{eq:mlp_regularization}) durch Gradientenverfahren reduziert:
\begin{itemize}
\item Vorw\"{a}rtslauf: Berechnung der Netzausgabe $\hat{y}_i$.
\item R\"{u}ckw\"{a}rtslauf: Backpropagation der Gradienten.
\item Update: z.\,B.\ Adam oder SGD, gesteuert \"{u}ber \texttt{learning\_rate\_init}.
\end{itemize}

\subsubsection{4) Konvergenz, Abbruch und typische Diagnose}
Das Training endet, wenn \texttt{max\_iter} erreicht ist oder wenn die Verbesserung der
Loss unter eine Toleranz f\"{a}llt. Typische Diagnoseindikatoren sind:
\begin{itemize}
\item Trainingsfehler sinkt, Testfehler steigt $\Rightarrow$ Overfitting
      (Regularisierung erh\"{o}hen, Netz verkleinern oder mehr Daten).
\item Beide Fehler hoch $\Rightarrow$ Underfitting
      (mehr Kapazit\"{a}t oder andere Aktivierung).
\item Stagnation $\Rightarrow$ Lernrate, Aktivierung oder Skalierung pr\"{u}fen.
\end{itemize}

\subsubsection{5) Hyperparameter-Optimierung mittels Grid-Search und CV}
Im L2ML-Fitting-Tool k\"{o}nnen MLP-Hyperparameter wie \texttt{hidden\_layer\_sizes},
\texttt{alpha} und \texttt{learning\_rate\_init} systematisch per Grid-Search mit
$k$-facher Kreuzvalidierung durchsucht werden. Da das Trainingsergebnis vom zuf\"{a}lligen
Anfangszustand abh\"{a}ngt, empfiehlt sich die Mittelung \"{u}ber mehrere CV-Folds.

\begin{hinweisbox}
Im Gegensatz zu LOLIMOT/HILOMOT sind beim MLP die Hyperparameter nicht nur strukturell,
sondern bestimmen direkt das Optimierungsverhalten (Konvergenzgeschwindigkeit und
Stabilit\"{a}t). Eine sorgf\"{a}ltige Hyperparameterwahl ist daher besonders wichtig.
\end{hinweisbox}
% ------------------------------------------------------------
...
Im L2ML-Fitting-Tool wird scikit-learn's \texttt{SVR} eingesetzt \cite{sklearn_svr, pedregosa2011}.
...
% ------------------------------------------------------------
\subsection{Fit-Prozess im Detail: Support Vector Regression}
\label{sec:svr_fit_process}

Der Fit-Prozess der SVR besteht in der L\"{o}sung eines konvexen Optimierungsproblems.
Anders als beim MLP gibt es (bei fester Kernelwahl) keine lokalen Minima: Der Optimierer
findet eine globale L\"{o}sung.

\subsubsection{1) Skalierung als Voraussetzung}
SVR ist stark skalenabh\"{a}ngig, insbesondere beim RBF-Kernel
$K(\mathbf{x},\mathbf{x}') = \exp(-\gamma \lVert \mathbf{x}-\mathbf{x}'\rVert^2)$:
\begin{itemize}
\item Ohne Standardisierung dominieren Eingansgr\"{o}\ss{}en mit gro\ss{}er Varianz.
\item $\gamma$ (bzw.\ die Kernelbreite) ist sonst nicht sinnvoll interpretierbar.
\end{itemize}

\subsubsection{2) Hyperparameterwahl mittels Grid-Search und CV}
Die wichtigsten Parameter und ihr Fit-Effekt:
\begin{itemize}
\item \textbf{$C$}: Trade-off zwischen Modellgl\"{a}tte und Fehlerstrafe. Kleines $C$:
      mehr Support Vectors, gl\"{a}tteres Modell. Gro\ss{}es $C$: enger Fit.
\item \textbf{$\varepsilon$}: Breite der $\varepsilon$-Tube; gr\"{o}\ss{}eres $\varepsilon$
      $\Rightarrow$ weniger Support Vectors, gl\"{a}tteres Modell.
\item \textbf{$\gamma$} (RBF): bestimmt die effektive Lokalit\"{a}t des Modells;
      zu gro\ss{} $\Rightarrow$ Overfitting, zu klein $\Rightarrow$ Underfitting.
\end{itemize}
Im L2ML-Fitting-Tool wird die Hyperparameter-Kombination $(C, \varepsilon, \gamma)$ typisch
per \texttt{GridSearchCV} mit $k$-facher Kreuzvalidierung bestimmt, um das
Bias-Varianz-Gleichgewicht datengetrieben zu optimieren.

\subsubsection{3) L\"{o}sung des dualen Problems und Support Vectors}
Beim Fit werden die Lagrange-Multiplikatoren $\alpha_i, \alpha_i^*$ bestimmt.
Nur f\"{u}r Punkte mit $\alpha_i - \alpha_i^* \neq 0$ entstehen Support Vectors.
Die Vorhersagefunktion ergibt sich anschlie\ss{}end als Summe \"{u}ber Support Vectors:
\begin{equation}
\hat{y}(\mathbf{x}) = \sum_{i \in \mathrm{SV}} (\alpha_i-\alpha_i^*)\,K(\mathbf{x}_i,\mathbf{x}) + b
\end{equation}

\subsubsection{4) Diagnose-Effekte im Tool}
Typische Effekte im Fit:
\begin{itemize}
\item Sehr viele Support Vectors $\Rightarrow$ h\"{a}ufig zu gro\ss{}es $C$ oder zu
      kleines $\varepsilon$.
\item Sehr ``zitternde'' Oberfl\"{a}che $\Rightarrow$ $\gamma$ zu gro\ss{} (Overfitting).
\item Zu glatte Oberfl\"{a}che $\Rightarrow$ $\gamma$ zu klein oder $C$ zu klein
      (Underfitting).
\end{itemize}
% ------------------------------------------------------------
...
Im L2ML-Fitting-Tool wird scikit-learn's \texttt{GaussianProcessRegressor} verwendet \cite{sklearn_gpr, pedregosa2011}.
...
% ------------------------------------------------------------
\subsection{Fit-Prozess im Detail: Gaussian Process Regression}
\label{sec:gpr_fit_process}

Bei der Gau\ss{}prozessregression ist das Training im Kern die Anpassung der
Kernel-Hyperparameter (z.\,B.\ L\"{a}ngenma\ss{}st\"{a}be) sowie ggf.\ eines Rauschterms.
Anschlie\ss{}end erfolgt die Vorhersage \"{u}ber posteriore Verteilungen.

\subsubsection{1) Skalierung und Kernel-Initialisierung}
Auch GPR profitiert stark von standardisierten Eing\"{a}ngen. Kernel-Hyperparameter
(z.\,B.\ L\"{a}ngenma\ss{}st\"{a}be $\ell$) beziehen sich direkt auf Distanzen im Eingangsraum
und sind daher bei unstandardisierten Daten schwer zu interpretieren.

\subsubsection{2) Maximierung der log-marginalen Likelihood}
F\"{u}r gegebene Hyperparameter $\boldsymbol{\theta}$ ergibt sich die Kovarianzmatrix
$K_\theta(X,X)$. Die Hyperparameter werden durch Maximierung der log-marginalen Likelihood
gesch\"{a}tzt (Abschnitt~\ref{sec:gpr_fit_process}):
\begin{equation}
\log p(\mathbf{y}\mid X,\boldsymbol{\theta})
=
-\frac{1}{2}\mathbf{y}^T(K_\theta+\sigma_n^2 I)^{-1}\mathbf{y}
-\frac{1}{2}\log|K_\theta+\sigma_n^2 I|
-\frac{N}{2}\log(2\pi)
\end{equation}
Dies balanciert Datenanpassung (erster Term) und Modellkomplexit\"{a}t (Log-Determinante).
Mehrere Neustarts (\texttt{n\_restarts\_optimizer}) helfen, lokale Maxima zu vermeiden.

\subsubsection{3) Posterior-Verteilung und Unsicherheitsquantifizierung}
Nach dem Fit liefert GPR nicht nur einen Mittelwert $\mathbb{E}[y^*]$
(Gleichung~\ref{eq:gpr_mean}), sondern auch eine Varianz $\mathrm{Var}[y^*]$
(Gleichung~\ref{eq:gpr_variance}) als Unsicherheitsma\ss{}.
Diese Information ist besonders wertvoll bei Extrapolation oder d\"{u}nn besetzten
Bereichen des Eingangsraums.

\subsubsection{4) Hyperparameter-Optimierung und Grid-Search}
W\"{a}hrend die Kernelparameter bei GPR automatisch \"{u}ber die marginale Likelihood gesch\"{a}tzt
werden, k\"{o}nnen strukturelle Entscheidungen wie die Kernelwahl oder die Anzahl der
Neustarts (\texttt{n\_restarts\_optimizer}) sowie die maximale Datenpunktanzahl
(\texttt{max\_fit\_samples}) im L2ML-Fitting-Tool ebenfalls per Grid-Search und
Kreuzvalidierung evaluiert werden.

\begin{merkbox}
GPR unterscheidet sich im Fit-Prozess fundamental von MLP/SVR: Es wird keine Loss \"{u}ber
Gewichte minimiert, sondern eine probabilistische Evidenz (marginale Likelihood) optimiert,
und das Modell liefert zus\"{a}tzlich Konfidenzintervalle.
\end{merkbox}
% ------------------------------------------------------------
...
auf einem einheitlichen Datensatz. Es verwendet f\"{u}r HILOMOT/LOLIMOT eine
...
\item SVR: $C$, $\gamma$ und $\varepsilon$ interagieren stark; Kreuzvalidierung empfiehlt sich zur Parameteroptimierung.
